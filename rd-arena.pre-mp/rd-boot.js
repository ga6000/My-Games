...
function loop() {
    ...
    // The packet clock. Repacks on RDNET.INTERVAL_MS and advances the morph;
    // driven from the loop rather than a timer, so it cannot outlive destroy().
    ...
    //
    // 2026-09-07: the cells drawn are now the 133^2 2-bit NETWORK field
    // (rd-netfield.js, NET_FIELD_NOTES.md) morphing between 1 s packets, not
    // gridB -- so what you look at is what a client would look at. gridB is
    // still what you WALK INTO; nothing below reads it. F9 flips back.
    if (RDNET.enabled) {
        const res = RDNET.RES, nc = RDNET.cell;
        const T = RDNET.threshold(nowMs);
        let nx0 = Math.max(0, Math.floor(cameraX / nc));
        let ny0 = Math.max(0, Math.floor(cameraY / nc));
        let nx1 = Math.min(res, Math.ceil((cameraX + canvas.width) / nc));
        let ny1 = Math.min(res, Math.ceil((cameraY + canvas.height) / nc));

        ctx.fillStyle = RDSKIN.fleshCold;
        for (let x = nx0; x < nx1; x++) {
            for (let y = ny0; y < ny1; y++) {
                const b = RDNET.sample(x + y * res);
                if (b > T && b <= RDSKIN.HOT_BAND) ctx.fillRect(x * nc, y * nc, nc, nc);
            }
        }
        ctx.fillStyle = RDSKIN.fleshHot;
        for (let x = nx0; x < nx1; x++) {
            for (let y = ny0; y < ny1; y++) {
                if (RDNET.sample(x + y * res) > RDSKIN.HOT_BAND) ctx.fillRect(x * nc, y * nc, nc, nc);
            }
        }
    } else {
